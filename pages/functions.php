...
function register($name, $pass, $email)
{
    ...
    while ($line = fgets($file, 128)) {
        ...
        if ($readname == $name) {
            echo "<h3/><span style='color:red;'>
                     Such Login Name Is Already Used!
                </span><h3/>";
            return false;
        }
    }
...
function is_login($name, $passw)
{
    //data validation block
    $name = trim(htmlspecialchars($name));
    $passw = md5(trim(htmlspecialchars($passw)));
    //login&pasw uniqueness check block
    global $users;
    $file = fopen($users, 'r');

    while ($line = fgets($file, 128)) {
        $readname = substr($line, 0, strpos($line, ':'));
        $readpswd = substr($line, strpos($line, ':') + 1, strlen($line) - (strpos($line, ':') + 1) - (strlen($line) - strrpos($line, ':')));
        if ($readname == $name && $readpswd == $passw) {
            $_SESSION['registred_user'] = $name;
            return true;
        }
    }
